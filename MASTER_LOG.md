...
Post-reconciliation live run: operator re-ran `scripts/install-ryzen-smu-dkms.sh` on the 5950X host; the amkillam clone SUCCEEDED but `dkms add` / `dkms install` failed with "Arguments <module> and <module-version> are not specified." Two independent root causes, two merged chunks:
