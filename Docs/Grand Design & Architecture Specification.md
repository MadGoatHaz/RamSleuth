...
- Primary dev host: **AMD Ryzen 9 5950X (Zen 3, Vermeer), 16C/32T, 64 MiB L3, DDR4-3600 (a G.Skill F4-3600C18-32GVK kit — 2× 16 GiB rank-1; the SPD's 3200 MT/s *base* speed was the source of the earlier "DDR4-3200" misnomer), AVX2; CachyOS, kernel `7.2.3-1-cachyos-custom`** — `ryzen_smu` is **NOW INSTALLED + LOADED** there (amkillam/ryzen_smu v0.1.7, DKMS — `dkms status` = `ryzen_smu/1.d298366, 7.2.3-1-cachyos-custom, x86_64: installed`; sysfs at `/sys/kernel/ryzen_smu_drv/`, incl. the `smn` attr). Live AMD telemetry **fully works (Cycle 5)**: PM clocks = 1800 MHz OneToOne + VDDCR_SOC = 1.1375 V (the reconciled Vermeer f32 PM model) + **the 27 DRAM timings + GDM live from the SMN `smn` attr** (tick-identical to `monitor_cpu -m` — the ground-truth cross-check returned **VERDICT PASS**, 31 active gates; CAD/RTT/drive honest `Na` pending driver-side bitfield confirmation). Install/management = the idempotent `scripts/install-ryzen-smu-dkms.sh` helper (DKMS staging + build + install + `modprobe` + verify; `AUTOINSTALL=yes` rebuilds on kernel updates) — documented in `Docs/HANDOVER.md` §6 (and `Docs/RamSleuth-v2.md` §"Verification Environment"). Without the module the AMD sections render `N/A (DriverMissing)` (graceful, no panic).
