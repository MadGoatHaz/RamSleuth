...
Status (2026-09-15 — Cycle 5 close-out)
...
- Phase 6 — Live-Hardware Verification & Remaining Reconciliation: **COMPLETE** (Cycle 5, 8 chunks P6-01…P6-11: the AMD ground truth **CLOSED with VERDICT PASS** — 31 active gates in tolerance (clocks ±1 MHz, voltages ±10 mV, 27/27 DRAM timings tick-identical to the `monitor_cpu` reference, GDM triple-confirmed); the SMN 27-timings + GDM path live; the SPD `0x0D`/`0xC1` reconciliation; the L1/L2 inner-loop iterations; P6-06 Intel parked hardware-gated, P6-07/08 operator-gated not authored).
- Workspace QA baseline (2026-09-15): **371/371 tests green (debug + release), clippy `-D warnings` clean, MSRV 1.75 (lockfile-pinned), 6 release binaries, `Cargo.lock` untouched.** `ryzen_smu` (amkillam v0.1.7) is installed + loaded on the AMD dev host — live AMD telemetry is fully working (PM clocks 1800 MHz + VDDCR_SOC 1.1375 V + the 27 DRAM timings + GDM from the SMN registers). **Ready for Cycle 6** (the GUI workstream is primary per the operator; O2/O5/O6 operator/hardware-gated — see `Docs/HANDOVER.md` §5).
...
(a) **Primary AMD dev host — Ryzen 9 5950X (Zen 3, Vermeer), 16C/32T, 64 MiB L3, DDR4-3600 (a G.Skill F4-3600C18-32GVK kit — 2× 16 GiB rank-1; the SPD's 3200 MT/s *base* speed was the source of the earlier "DDR4-3200" misnomer), AVX2 (no AVX-512); CachyOS, kernel `7.2.3-1-cachyos-custom`.**
...
- Consequence: live AMD telemetry **fully works (Cycle 5)** — the daemon decodes the PM table (MCLK/UCLK/FCLK = 1800 MHz OneToOne, VDDCR_SOC = 1.1375 V) **plus the 27 DRAM timings + GDM from the SMN registers** (CAD/RTT/drive honest `Na` pending driver-side bitfield confirmation). The tick-identical ground-truth gate is **CLOSED** (open item 1: the cross-check vs `sudo monitor_cpu` returned **VERDICT PASS — 31 active gates in tolerance**; `monitor_cpu` is at `/usr/bin/monitor_cpu`, mode 700 root — note its PM-frame path requires `-f` on this 0x380805 table; the cross-check script is `scripts/amd-ground-truth.sh`).
...
- **Current state (2026-09-15, supersedes the above snapshots):** `ryzen_smu` is installed + loaded on this host (amkillam/ryzen_smu v0.1.7, DKMS) — the AMD section is **populated live end-to-end** (clocks = 1800 MHz OneToOne + VDDCR_SOC = 1.1375 V, from the reconciled Vermeer f32 PM-table model — P5-15; **the 27 DRAM timings + GDM live from the SMN registers — P6-02/03/10**; CAD/RTT/drive honest `Na` pending driver-side bitfield confirmation); the tick-identical ground-truth cross-check is **CLOSED** (open item 1: `scripts/amd-ground-truth.sh` → **VERDICT PASS** — 31 active gates in tolerance vs `monitor_cpu`). Intel live decode still pending the i5-6600 (open item 2, hardware-gated — the machine is not yet attached); model reconciliation done except the CAD/RTT/drive bitfields (driver-side) + the Intel IMC offsets (hardware-gated). See `Docs/HANDOVER.md` §4–§5.
