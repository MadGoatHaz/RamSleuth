...
- **GUI (`ramsleuth-gui`):** an egui/eframe desktop dashboard — a three-zone 968×600 window (live telemetry matrix, benchmark grid, hardware/SPD status), a dedicated Graphs window, a Settings panel, `[F2]` PNG snapshot / `[F3]` JSON export / `[Q]`uit, repainting at ~60 FPS.
...
| `ramsleuth-gui` | `--socket <path>`; keys `[F2]` PNG snapshot / `[F3]` JSON export / `[Q]`uit | 0 quit · 1 eframe/display failure · 2 usage |
...
2. **Clients.** Grant a normal user access to the `0660` group-owned socket:
   ...
   ramsleuth-gui              # the live desktop dashboard
   ```
...
- **From a source checkout:** `scripts/install-ryzen-smu-dkms.sh` — idempotent, re-execs under `sudo`, requires the matching kernel headers, clones the verified upstream `amkillam/ryzen_smu` (branch `main`, v0.1.7; overridable via `RYZEN_SMU_URL`), then `dkms install` + `modprobe` + a persistent `/etc/modules-load.d/ryzen_smu.conf`. `AUTOINSTALL=yes` rebuilds the module on kernel updates.
