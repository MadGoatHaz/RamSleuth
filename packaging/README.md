...
Operator/end-user guide for installing and running RamSleuth v2: the AUR package model — the **two main packages** (`ramsleuth` stable source, `ramsleuth-bin` precompiled), which **both bundle the in-repo `ramsleuth_intel` DKMS source** (Intel — a first-class, validated feature), plus the **two optional vendor DKMS extras** — `ryzen-smu-dkms` (the **offline** `ryzen_smu` module, AMD) and `ramsleuth-intel-dkms` (the standalone provisioning path for the Intel module — **mutually exclusive** with the main packages, which already carry its source) — **one-click setup** (no re-login, no reboot), the `ramsleuth` group, the sandboxed systemd unit, the unified version bump, and CI.
...
`ramsleuth-protocol` is a library-only crate and is never installed. Both packages install the two one-click artifacts (the helper + polkit policy): `ramsleuth` builds them from source, and `ramsleuth-bin` takes them from the release tarball (which has carried them since the v2.2.0 re-cut). Both also install the ramsleuth-owned copies of the two DKMS helpers (the `/usr/bin/ramsleuth-install-ryzen-smu-dkms` / `/usr/bin/ramsleuth-install-intel-dkms` pair — see the name table in the AMD extra section below), and **both bundle the in-repo `ramsleuth_intel` DKMS source tree** to `/usr/share/ramsleuth-intel-dkms/src/` (the exact path the Intel helper resolves, so the one-click Intel DKMS install works from a bare AUR install with no manual source step); the Intel helper and the bundled source are guarded the same way — they are in the current source tree and in the v2.4.5 `-bin` release tarball (the `ramsleuth` source build installs them), and the guard covers pre-2.4.2 tags and the old published v2.2.1 tarball (the `ramsleuth-bin` asset), where the install is skipped cleanly.
...
- **The two vendor extras** — **`ryzen-smu-dkms`** (AMD) and **`ramsleuth-intel-dkms`** (Intel): optional, thin provisioning packages that build the matching DKMS module **on the target** (never in the build chroot) and install their own copy of the operator helper under a distinct name. Install the one that matches your silicon — the full name table lives in the AMD extra section below. The AMD extra stays **co-install-safe** with both main packages (no pacman file conflict). The Intel extra is different: **both main packages now bundle the in-repo `ramsleuth_intel` source tree** (to `/usr/share/ramsleuth-intel-dkms/src/` — the exact path the Intel helper resolves), so the extra declares `conflicts=` against both mains (and they against it) — it is **mutually exclusive** with them: installing it removes a main package first, and for Intel it is now the standalone/redundant provisioning path (the module itself is a first-class, validated feature — in-repo source, built warning-free by the CI `kernel-module` job, 25 frozen sysfs attributes).

**Mutual conflict:** `ramsleuth` and `ramsleuth-bin` install the identical file set, so each declares the other in `conflicts=` — the user picks exactly one. The AMD extra (`ryzen-smu-dkms`) stays co-install-safe with both (its helper ships under a distinct name and it shares no file with the mains); the Intel extra (`ramsleuth-intel-dkms`) **conflicts with both mains** (and they with it) — the shared `/usr/share/ramsleuth-intel-dkms/src/` source tree the mains now bundle — so it can only be installed after removing a main package.
...
| `scripts/install-intel-dkms.sh` (Intel) | `/usr/bin/ramsleuth-install-intel-dkms` | `install.sh` and both ramsleuth AUR packages (guarded — the helper is in the current source tree and in the v2.4.5 `-bin` release tarball; the guard covers pre-2.4.2 tags and the old v2.2.1 tarball) — this is the name `ramsleuth-setup --with-dkms` (the Intel arm) and `--with-intel-dkms` delegate to |
| (same Intel script) | `/usr/bin/ramsleuth-intel-dkms-install` | the standalone `ramsleuth-intel-dkms` AUR extra only |

The distinct names are deliberate: for the AMD extra they keep it **co-install-safe** with any ramsleuth package (no pacman file conflict). The Intel extra is no longer co-installable with the mains (they bundle the same `/usr/share/ramsleuth-intel-dkms/src/` source tree, so pacman enforces the mutual conflict by file). The two names of one script behave identically — use the one your install provides.
...
Build posture: the `ramsleuth` package compiles **only this repository** (tag-pinned, `--locked`, no third-party code in the build chroot), and `ramsleuth-bin` compiles nothing — it downloads the release tarball pinned by `sha256sums`. Neither ramsleuth package depends on **any third-party AUR package** — the AMD extra is optional and co-install-safe (it ships its helper under a distinct name, so it never conflicts with the RamSleuth packages), while the Intel extra is mutually exclusive with the mains (shared source-tree path). GPL-2.0 module source ships **only** via the matching package — the vendored `ryzen_smu` via `ryzen-smu-dkms` (the release tarball deliberately excludes its vendor dir for size), the in-repo `ramsleuth_intel` tree via **both** the `ramsleuth-intel-dkms` extra **and the two main packages** (which bundle it to `/usr/share/ramsleuth-intel-dkms/src/`; their `license=(MIT GPL-2.0-only)` reflects that — the bundled tree is a separate work, never compiled into any RamSleuth binary). Every installed file is byte-identical to a file in this repository (auditable via `git show`), including the shipped helpers and `install.sh` (`/usr/share/ramsleuth/install.sh`), so the whole flow can be re-run and audited post-install.

### The release → AUR lockstep (standing process)

The v2.2.0 re-cut is **done** (the one-click helper + polkit policy now ship in every tarball and source build), and the workspace is at **v2.4.5** (the tag + release re-cut are operator-gated), in which the Intel helper (`scripts/install-intel-dkms.sh`) + the `kernel/ramsleuth-intel/` module tree are in the source tree. The **published** `ramsleuth-bin` asset is still **v2.2.1** until the v2.4.5 re-cut is published; after the v2.4.5 tag, the published asset is **v2.4.5** and its tarball **carries** the Intel helper (`scripts/install-intel-dkms.sh`) + the `kernel/ramsleuth-intel/` module tree — per the 19-artifact release contract (19 files: 6 binaries + 13 auxiliary: the 8 pre-Intel auxiliary files, the Intel DKMS helper, and the 4 module source files; the `assets/icons/` hicolor tree ships as a bundle) — so the `-bin` one-click Intel path is fully active (the v2.2.1 tarball carries only the AMD helper). Both ramsleuth packages **guard** the Intel helper + source-tree install, and the guard now covers only pre-2.4.2 assets (a build against a pre-2.4.2 tag or the old published v2.2.1 tarball skips it cleanly). The standing process for **each** release is:
...
There is no pin, no clone, no vendor-SUMS manifest: there is no upstream to track (no `RYZEN_SMU_*`-class env overrides apply). The DKMS version is the ramsleuth workspace version (a fixed version — `2.4.5` today — which makes `dkms add` idempotent across kernel updates).
...
1. One of:
   - `sudo ramsleuth-setup --with-dkms` — the one-click path (vendor-aware: on Intel silicon it delegates to the installed Intel helper); `sudo ramsleuth-setup --with-intel-dkms` forces the Intel arm (the Intel-only fast path). On a current **v2.4.5** install the ramsleuth-owned Intel helper ships in the source tree (the published `ramsleuth-bin` asset is still v2.2.1 until the re-cut), and **both main packages bundle the module source**, so the one-click works from a bare AUR install with no manual source step — the AUR extra below is the standalone alternative (mutually exclusive with the mains);
