...
| `scripts/install-intel-dkms.sh` (Intel) | `/usr/bin/ramsleuth-install-intel-dkms` | `install.sh` and both ramsleuth AUR packages (guarded — the helper is in the v2.4.2 source tree; the guard covers pre-2.4.2 tags and the published v2.2.1 tarball) — this is the name `ramsleuth-setup --with-dkms` (the Intel arm) and `--with-intel-dkms` delegate to |
...
The v2.2.0 re-cut is **done** (the one-click helper + polkit policy now ship in every tarball and source build), and the workspace is at **v2.4.2** (the tag + release re-cut are operator-gated), in which the Intel helper (`scripts/install-intel-dkms.sh`) + the `kernel/ramsleuth-intel/` module tree are in the source tree. The **published** `ramsleuth-bin` binary is still the **v2.2.1** asset until the v2.4.2 re-cut is published, so the current `v2.2.1` tarball carries only the AMD helper, and both ramsleuth packages **guard** the Intel helper install for pre-2.4.2 tags/tarballs (a build against a pre-2.4.2 tag or the current `-bin` tarball skips it cleanly; the `-bin` one-click Intel path self-heals when the v2.4.2 re-cut lands). The standing process for **each** release is:
...
There is no pin, no clone, no vendor-SUMS manifest: there is no upstream to track (no `RYZEN_SMU_*`-class env overrides apply). The DKMS version is the ramsleuth workspace version (a fixed version — `2.4.2` today — which makes `dkms add` idempotent across kernel updates).
...
1. One of:
   - `sudo ramsleuth-setup --with-dkms` — the one-click path (vendor-aware: on Intel silicon it delegates to the installed Intel helper); `sudo ramsleuth-setup --with-intel-dkms` forces the Intel arm (the Intel-only fast path). On a current **v2.4.2** install the ramsleuth-owned Intel helper ships in the source tree (the published `ramsleuth-bin` asset is still v2.2.1 until the re-cut), and **both main packages bundle the module source**, so the one-click works from a bare AUR install with no manual source step — the AUR extra below is the standalone alternative (mutually exclusive with the mains);
