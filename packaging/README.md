...
| `scripts/install-intel-dkms.sh` (Intel) | `/usr/bin/ramsleuth-install-intel-dkms` | `install.sh` and both ramsleuth AUR packages (guarded — the helper is in the v2.4.5 source tree; the guard covers pre-2.4.5 tags and the published v2.2.1 tarball) — this is the name `ramsleuth-setup --with-dkms` (the Intel arm) and `--with-intel-dkms` delegate to |
...
The v2.2.0 re-cut is **done** (the one-click helper + polkit policy now ship in every tarball and source build), and the workspace is at **v2.4.5** (the tag + release re-cut are operator-gated), in which the Intel helper (`scripts/install-intel-dkms.sh`) + the `kernel/ramsleuth-intel/` module tree are in the source tree. The **published** `ramsleuth-bin` binary is still the **v2.2.1** asset until the v2.4.5 re-cut is published, so the current `v2.2.1` tarball carries only the AMD helper, and both ramsleuth packages **guard** the Intel helper install for pre-2.4.5 tags/tarballs (a build against a pre-2.4.5 tag or the current `-bin` tarball skips it cleanly; the `-bin` one-click Intel path self-heals when the v2.4.5 re-cut lands). The standing process for **each** release is:
