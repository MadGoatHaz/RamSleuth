//! End-to-end `--version` / `--help` behavior of the `ramsleuth-telemetry` binary
//! (spawned via `CARGO_BIN_EXE_ramsleuth-telemetry` — the real bin, not the test
...
fn bin_path() -> std::path::PathBuf {
    ...
    // `<target>/debug/deps/cli-<hash>` -> sibling `<target>/debug/ramsleuth-telemetry>`.
    if let Some(path) = std::env::var_os("CARGO_BIN_EXE_ramsleuth-telemetry") {
    ...
    debug_dir.join("ramsleuth-telemetry")
}
...
fn run(args: &[&str]) -> std::process::Output {
    Command::new(bin_path())
        .args(args)
        .output()
        .expect("the ramsleuth-telemetry binary must spawn")
}
...
fn version_prints_the_package_version_and_exits_zero() {
    for flag in ["--version", "-V"] {
        ...
        let expected = format!("ramSleuth ramsleuth-telemetry v{}", env!("CARGO_PKG_VERSION"));
...
fn help_prints_the_usage_and_exits_zero() {
    for flag in ["--help", "-h"] {
        ...
        assert!(
            stdout.contains("Usage: ramsleuth-telemetry ["),
