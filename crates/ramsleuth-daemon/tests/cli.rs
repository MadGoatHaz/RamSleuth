//! End-to-end `--version` / `--help` behavior of the `ramsleuth-daemon` binary
//! (spawned via `CARGO_BIN_EXE_ramsleuth-daemon` — the real bin, not the test
...
fn bin_path() -> std::path::PathBuf {
    ...
    // `<target>/debug/deps/cli-<hash>` -> sibling `<target>/debug/ramsleuth-daemon>`.
    if let Some(path) = std::env::var_os("CARGO_BIN_EXE_ramsleuth-daemon") {
    ...
    debug_dir.join("ramsleuth-daemon")
}
...
fn run(args: &[&str]) -> std::process::Output {
    Command::new(bin_path())
        .args(args)
        .output()
        .expect("the ramsleuth-daemon binary must spawn")
}
...
fn version_prints_the_package_version_and_exits_zero() {
    for flag in ["--version", "-V"] {
        ...
        let expected = format!("ramSleuth ramsleuth-daemon v{}", env!("CARGO_PKG_VERSION"));
...
fn help_prints_the_usage_and_exits_zero() {
    for flag in ["--help", "-h"] {
        ...
        assert!(
            stdout.contains("Usage: ramsleuth-daemon ["),
