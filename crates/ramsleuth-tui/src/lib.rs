...
//! the units / refresh / window cycles — the full 16-key table in
//! [`events`]).
//!
//! Modules land one per chunk (plan §3, group E):
//!
//! - `events` — P3-22 + TUI-01/02 — the frozen input contract: the pure
//!   `key_to_action` mapping (`crossterm::event::KeyEvent` -> `Action`,
//!   the full 16-key parity table, testable without a terminal) +
...
//!   16-key dispatch (the P3-22 trio byte-for-byte).
