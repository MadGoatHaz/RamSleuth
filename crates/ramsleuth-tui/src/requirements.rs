...
//! three cases, TUI-`AppState` flavored:
//!
...
/// Cyan: the commands to run (the "do this" affordance).
...
/// The host's CPU vendor for the AMD-branch check (the GUI
...
/// (the GUI `first_run::diagnose` mirror, the three cases in the
...
// ---------------------------------------------------------------------
// Tests (headless: the five `diagnose` cases from the plan + the
// strip's no-panic render over ratatui's in-memory TestBackend — the
// ui.rs test idiom; they run once TUI-23 wires the module).
// ---------------------------------------------------------------------
...
mod tests {
    ...
    /// (host-independent — the GUI first_run test pattern).
    fn snapshot(vendor: CpuVendor, amd: NaReason) -> SystemMemoryTelemetry {
    ...
    fn connected(vendor: CpuVendor, amd: NaReason) -> AppState {
        AppState {
            telemetry: Some(snapshot(vendor, amd)),
    ...
    /// MCHBAR decode, no extra driver; the AMD branch
    /// non-`DriverMissing`) → zero requirements.
    #[test]
    fn diagnose_connected_clean() {
        let state = connected(CpuVendor::Intel(IntelGen::Skylake), NaReason::NotApplicable);
        assert!(
    ...
    fn diagnose_amd_driver_missing() {
        let state = connected(CpuVendor::Amd(AmdZen::Zen3), NaReason::DriverMissing);
        let requirements = diagnose(&state);
        assert_eq!(
            requirements.len(),
            1,
    ...
    /// `Na(DriverMissing)` and the daemon connected → no DKMS
    /// requirement (the vendor gate, the GUI first_run (d) mirror).
    #[test]
    fn diagnose_non_amd_driver_missing() {
        let state = connected(CpuVendor::Intel(IntelGen::Skylake), NaReason::DriverMissing);
        assert!(
            diagnose(&state).is_empty(),
    ...
    /// (f) The strip's no-panic render (the ui.rs TestBackend idiom):
